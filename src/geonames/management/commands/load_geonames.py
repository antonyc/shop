...
def get_cmd_options():
    ...
    return "-u %s -p%s %s" % (db_settings['USER'],
...
class Command(NoArgsCommand):
    ...
    def handle_noargs(self, **options):
        ...
        fromfile_cmd = 'mysql %(db_opts)s << %(sql_file)s'
        ...
        # reduces disk I/O.
        copy_sql = "INSERT %s (geonameid,name,alternates,fclass,fcode,country,cc2,admin1,admin2,admin3,admin4,population,elevation,topo,timezone,moddate,point) FROM STDIN;" % db_table
        copy_cmd = 'zcat %(gz_file)s | mysql %(db_opts)s -e "%(copy_sql)s"'
        copy_args = {'gz_file' : os.path.join(GEONAMES_DATA, 'allCountries.gz'),
                     'db_opts' : db_opts,
                     'copy_sql' : copy_sql
                     }

        fp_sql = open('/tmp/mysql_geonames.sql', 'w')
        fp = open('/tmp/mysql', 'r')
